...
%}]
 ...
 \begin{table}[!ht]
   \centering
  \caption{ROC and Confidence Bounds}
\input{res}
\end{table}
...
\node[anchor=west] (b) at ([yshift=0in]a.east) {\includegraphics[width=\WDT]{Figures/sczVscaff.pdf}};
\node[anchor=north] (c) at ([yshift=-.2in]a.south) {\includegraphics[width=\WDT]{Figures/sczsceff.pdf}};
\node[anchor=south west] (L1) at (a.north west) {\Large a.};
\node[anchor=south west] (L2) at (b.north west) {\Large b.};
\node[anchor=south west] (L3) at (c.north west) {\Large c.};

\node[anchor=west] (T) at (c.east) {
  \mnp{3.5in}{
\input{res}
  }
};
